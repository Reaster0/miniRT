...
/*   Updated: 2021/03/15 12:22:59 by earnaud          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int in_it(t_ray *ray, t_triangle *triangle, t_3d *inter_point, float *t)
{
	t_3d edge1;
	t_3d edge2;
	t_3d normal;
...
int inter_triangle2(t_ray *ray, t_triangle *triangle)
{
	...
	t_3d AI;
	float t;
	if (!in_it(ray, triangle, interp_norm, &t) || t <= 0.000001f || t >= ray->t)
		return (0);
	...
	AI = sub_product(interp_norm[0], triangle->a);
	...
	ray->shape_normale = interp_norm[1];
