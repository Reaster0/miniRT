...
void	inter_light(t_ray *ray, t_light *light, t_shapes *shapes)
{
	...
	{
		rgb[0] = intens_color(ray, light, get_r(ray->shape_color),
				get_r(ray->color), (light->intens / 255) * get_r(light->color));
		rgb[1] = intens_color(ray, light, get_g(ray->shape_color),
				get_g(ray->color), (light->intens / 255) * get_g(light->color));
		rgb[2] = intens_color(ray, light, get_b(ray->shape_color),
				get_b(ray->color), (light->intens / 255) * get_b(light->color));
